...
function includeFile(path, options) {
  var opts = shallowCopy({}, options || /* istanbul ignore next */ {});
  if (!opts.filename) {
...
exports.isCache = function (path) {
  return (jsCache[path] != null);
};
...
function Template(text, opts) {
  ...
  options.escapeFunction = opts.escape || escapeXML;
...
Template.prototype = new function () {
  ...
  this.createRegex = function () {
    var str = _REGEX_STRING
      , delim = escapeRegExpChars(this.opts.delimiter);
    ...
    if (matches && matches.length) {
      matches.forEach(function (line, index) {
        ...
        if ((include = line.match(/^\s*include\s+(\S+)/))) {
          includeOpts = shallowCopy({}, self.opts);
